...
## Code Style
- Использовать ES модули (ESM)
- Писать асинхронный код через async/await

## Workflow Rules
- Всегда запускать линтинг перед коммитом
- Не изменять файлы тестов без необходимости
```

Субагенты автоматически найдут этот файл и применят настройки.
