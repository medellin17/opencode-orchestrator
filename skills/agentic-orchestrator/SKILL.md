...
8. **Respect retry limits.** After 3 failed iterations, escalate to user.
9. **Synthesize, don't dump.** Final report must be concise and actionable.
10. **AGENTS.md (Project Context) Awareness:** Instruct sub-agents to look for `AGENTS.md` (or instructions from it in ~/.config/opencode/AGENTS.md) to load project-specific commands, style guides, and workflows. This promotes just-in-time context loading and avoids token bloat.
