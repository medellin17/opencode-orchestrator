...
6. **Refine plans** using the `plan-refiner` skill (`skill({ name: "plan-refiner" })`) if review issues are found.
7. **Maintain diagrams** using the `architecture-update` skill (`skill({ name: "architecture-update" })`) for C4/Mermaid updates.
